...
| RobBERT (line + filter) | **0.966** | 0.901 | 0.932 |
...
|---|---|---|
| Regex | **0.881** | **0.886** |
| RobBERT (unweighted) | 0.312 | 0.330 |
| RobBERT (weighted) | 0.465 | 0.858 |
| GPT-5.5 v3 (text) | 0.741 | 0.832 |
| GPT-5.5 v3 (vision+text) | **0.752** | 0.841 |

### Per-field breakdown

| Field | Regex F1 / ANLS* | RobBERT F1 / ANLS* | RobBERT (w) F1 / ANLS* | GPT text F1 / ANLS* | GPT vision F1 / ANLS* |
|---|---|---|---|---|---|
...
- **RobBERT (unweighted)**: Focal Loss only, no class weights. Collapses to
  F1=0.000 on CC, SUBJECT, and ATTACHMENT due to severe token-level class
  imbalance (O tokens = 70% of training data).
- **RobBERT (weighted)**: Same architecture with sqrt-inverse-frequency class
...
- **GPT-5.5 vision**: Stage 1 uses all PDF pages at 72 DPI + dossier text.
